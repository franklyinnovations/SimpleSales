Ext.define('SC.view.GrabNewWin',{
        ...
        tbar:[
            ...
            },
            {
              ...
              handler:function(){
                ...
                var gid = null;
                var url = require('url');
                var urlstr = Ext.getCmp('pro-name-new').getValue();
                if(urlstr == null){
                  return;
                }
                ...
                //var url = SC.component.LocalStorage.get('pro-url-new');
                me.app.sqlite.remove(null,gid,need_modify);
                Ext.create('SC.view.NotificationWin', {
											title: '删除单个商品',
											position: 'br',
											manager: 'grab-new-win',
              ...
              handler:function(){
                ...
                if(me.addedwinnew == null){
                  me.addedwinnew = Ext.create('Ext.window.Window', {
                      title: '添加结果',
                      height: 600,
                      width: 800,
                      layout: 'fit',
                ...
                me.app.sqlite.all(key,need_modify,pricestart,priceend,function(results){
                  ...
                  if(results.length > 0){
                    me.addedwinnew.title = '添加结果【总共:'+results.length+'条】';
              ...
              handler:function(){
                // var key = Ext.getCmp('pro-type-new').getValue();
                var tyname = Ext.getCmp('pro-type-new').getValue();
                var key = me.cateid;
  ...
  onDataviewSelected:function( thiz , record , eOpts ){
    ...
    if(record.data.url){
      Ext.getCmp('preview-panel-new').items.items[0].el.dom.src=record.data.url;
      Ext.getCmp('pro-name-new').setValue(record.data.url)
