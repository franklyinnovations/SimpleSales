Ext.define('SC.view.GrabWin',{
  ...
  createWindow:function(){
    ...
    var win = desktop.getWindow('grab-win');
    ...
    var cbox = Ext.create('Ext.form.ComboBox', {
        ...
        listeners:{
          select: function( combo , record , eOpts ){
            ...
            Ext.getCmp('pro-type').setValue(record.data.Name)
      ...
      win = desktop.createWindow({
        ...
        tbar:[
            ...
            },
            {
              ...
              handler:function(){
                  //me.selectedItem = null;
                SC.component.LocalStorage.set('pro-url','')
                SC.component.LocalStorage.set('pro-gid','')
                var keyword = Ext.getCmp('query-name').getValue();
                var pricestart = Ext.getCmp('price-start').getValue();
                var priceend = Ext.getCmp('price-end').getValue();
               ...
               if(!me.cateid){
                ...
                me.queryurl = "http://gz.17zwd.com/sks.htm?cateid=" + me.cateid + "&zdid=42&mid=-1&fid=-1&pstart="+pricestart+"&pend="+priceend+"&"+keywordparam+"&page=";
            ...
            },
            {
              ...
              text:'添加',
              handler:function(){
                //var key = Ext.getCmp('pro-type').getValue();
                var key = me.cateid;
                if(!key){
                  Ext.MessageBox.alert('添加商品','请选择分类');
                  return;
                }
                var gid =SC.component.LocalStorage.get('pro-gid');
                var url = SC.component.LocalStorage.get('pro-url');
                var need_modify = Ext.getCmp('need_modify').getValue() ? 1:0;
                var pricestart = Ext.getCmp('price-start').getValue();
                var priceend = Ext.getCmp('price-end').getValue();
                if(Number(pricestart) > Number(priceend)){
                  var temp = pricestart;
                ...
                me.app.sqlite.put(key,gid,url,need_modify,pricestart,priceend);
                SC.component.LocalStorage.set('pro-gid','');
                SC.component.LocalStorage.get('pro-url','');
                Ext.getCmp('need_modify').setValue(false);
                var tyname = Ext.getCmp('pro-type').getValue();
                Ext.create('SC.view.NotificationWin', {
											title: '添加商品',
											position: 'br',
											manager: 'grab-win',
              ...
              handler:function(){
                ...
                var gid = null;
                var url = require('url');
                var urlstr = Ext.getCmp('pro-name').getValue();
                if(urlstr == null){
                  return;
                }
                ...
                var gid = arg.GID;
                console.log('remove single:gid = ' + gid);
               var need_modify = Ext.getCmp('need_modify').getValue() ? 1:0;
                if(gid == null || gid == ''){
                  gid = null;
                  return;
                ...
                console.log('remove single:gid = ' + gid)
                me.app.sqlite.remove(null,gid,need_modify,null,null);
                Ext.create('SC.view.NotificationWin', {
											title: '删除单个商品',
											position: 'br',
											manager: 'grab-win',
                ...
                // me.app.sqlite.remove(null,gid,1,null,null);
              }
            },{
              xtype:'button',
              text:'标记',
              handler:function(){
                // if(me.markaddedwin == null){
              //  var markaddedwin = null;
                var markaddedwin = Ext.create('Ext.window.Window', {
                    title: '标记已经添加',
                    height: 600,
                    width: 800,
                    layout: 'fit',
                    ...
                    tbar:[{
                      ...
                      text:'标记',
                      handler:function(){
                        var value = Ext.getCmp('remove-batch-ta').getValue();
                        ...
                        results.forEach(function(result){
                          if(result != null && result.trim().length >= 0){
                            ...
                            var argstr = url.parse(result).query
                            var arg = querystring.parse(argstr);
                            var gid = arg.GID;
                            console.log('batch remove single:gid = ' + gid);
                            if(gid != null){
                              me.app.sqlite.markRemoved(gid);
              ...
              handler:function(){
                ...
                var need_modify = Ext.getCmp('need_modify').getValue() ? 1:0;
                //if(me.addedwin == null){
                  var addedwin = Ext.create('Ext.window.Window', {
                      title: '添加结果',
                      height: 600,
                      width: 800,
                      layout: 'fit',
                ...
                var ty = Ext.getCmp('pro-type').getValue();
                var resultsstr = '';
                me.app.sqlite.all(key,need_modify,pricestart,priceend,function(results){
                  if(results.length > 0){
                    addedwin.title = ty + ':添加结果【总共:'+results.length+'条】';
                    results.forEach(function(result){
                    //  console.log(result);
              ...
              var tyname = Ext.getCmp('pro-type').getValue();
                  ...
                  Ext.MessageBox.confirm('删除分类','确认删除【'+tyname+'】吗?',function(button,text){
                    if(button =='yes'){
                      var pricestart = Ext.getCmp('price-start').getValue();
                      var priceend = Ext.getCmp('price-end').getValue();
                      if(Number(pricestart) > Number(priceend)){
                      ...
                      var need_modify = Ext.getCmp('need_modify').getValue() ? 1:0;
                      me.app.sqlite.remove(key,null,need_modify,pricestart,priceend);
                      var tyname = Ext.getCmp('pro-type').getValue();
                      Ext.create('SC.view.NotificationWin', {
                            title: '删除商品分类',
                            position: 'br',
            ...
            },
            {
              ...
              text:'清空所有',
              handler:function(){
                Ext.MessageBox.confirm('清空所有','确认清空所有吗?',function(button,text){
                  if(button == 'yes'){
  ...
  onDataviewSelected:function( thiz , record , eOpts ){
    ...
    if(record.data.url){
      ...
      me.selectedItem = record.data;
      SC.component.LocalStorage.set('pro-url',record.data.url);
      SC.component.LocalStorage.set('pro-gid',record.data.gid);
    }
